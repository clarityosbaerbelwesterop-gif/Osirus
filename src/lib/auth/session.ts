...
export async function verifyBearer(token: string): Promise<AuthIdentity> {
  ...
  const { payload } = await jwtVerify(token, jwks, { clockTolerance: 5 });
  if (typeof payload.sub !== "string") throw new Error("invalid_session");
  return {
    userId: payload.sub,
